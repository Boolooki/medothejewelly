...
const SliderBanner: React.FC = () => {
  ...
  return (
    <div className="relative w-full h-[500px] overflow-hidden rounded-[10vw] mt-2">
      {/* Image Slides */}
      <div
      ...
      >
        {images.map((src, index) => (
          <div key={index} className="min-w-full h-[500px] relative">
            ...
            />
            {/* Text Content */}
            <div className="absolute bottom-4 right-4 bg-black text-white p-4 rounded">
              <h2 className="text-xl md:text-2xl font-bold">{textContent[index]}</h2>
              <a href='#' className="text-sm md:text-base hover:underline rounded mt-2 inline-block bg-white text-black px-3 py-1 text-black">
                Shop Now
              </a>
            </div>
